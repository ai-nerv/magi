...
-- Colours, as indices into the palette your terminal already has.
--
...
--
-- The defaults are the ordinary xterm reading — 1 red, 2 green, 3 yellow, 6 cyan, and 232-255 the
-- 24-step greyscale — because that is what a terminal nobody has configured actually holds.
--
--   axum.ui.accent      spinners, list cursors, bullets, inline code
--   axum.ui.success     added lines, a tool that finished
--   axum.ui.warning     warnings, elevated context
--   axum.ui.error       errors, removed lines, a tool that failed
--   axum.ui.heading     markdown headings
--   axum.ui.code        fenced code blocks
--   axum.ui.match       the characters you have typed, inside a completion
--   axum.ui.block_bg    behind a tool block and behind a menu
--   axum.ui.raised_bg   behind a user message, and the row you are on
--   axum.ui.rule        the rule above and below the prompt
--   axum.ui.dim         the footer
--   axum.ui.muted       tool output, quotes, reasoning, diff context
--   axum.ui.text        what the agent says
--   axum.ui.selected    the row you are on
--   axum.ui.border      the prompt's border at rest
--   axum.ui.scan        the brightest point of the light travelling along it
--
-- `border` and `scan` are the two ends of a gradient: the scan walks the indices between them,
-- so put them far enough apart to have something to walk.
--
-- axum.ui.accent = 1

-- A palette generated by lule is a different machine, and every default above is wrong on it.
-- Its slots 1-6 are the six most chromatic pigments of a wallpaper in chroma order — no hue
