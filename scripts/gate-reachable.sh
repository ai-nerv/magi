...
set -euo pipefail
reachable=$(cargo tree -p axum-cli --prefix none --no-dedupe 2>/dev/null \
  | awk '{print $1}' | grep '^axum-' | sort -u)
members=$(cargo metadata --no-deps --format-version 1 2>/dev/null \
  | grep -o '"name":"axum-[a-z-]*"' | cut -d'"' -f4 | sort -u)
fail=0
for member in $members; do
  # The testkit is the one exemption: it exists to be depended on by tests.
  [ "$member" = "axum-testkit" ] && continue
  if ! echo "$reachable" | grep -qx "$member"; then
    echo "$member is not reachable from the axum binary"
    fail=1
  fi
done
exit $fail
