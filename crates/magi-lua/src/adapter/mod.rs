...
use magi_provider::api::{Adapter, Delta, Options, StreamState};
...
/// lives in `magi_provider::compat::Resolved` and nowhere else — ten descriptions each
...
fn described(model: &Model) -> serde_json::Value {
    ...
    if let Some(object) = value.as_object_mut() {
        let resolved = magi_provider::compat::resolve(model.compat);
        object.insert(
            "compat".to_owned(),
...
fn effective_thinking(model: &Model, options: &Options) -> Option<String> {
    ...
    if level == magi_model::ThinkingLevel::Off {
        return None;
    }
