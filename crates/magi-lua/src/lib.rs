//! The Lua VM magi runs on, and the config API it offers `init.lua`.
//!
...
//!
//! magi.model = "anthropic/claude-sonnet-4-5"   -- a setting, assigned
//! magi.scan_speed = 2                          -- another one
//!
//! magi.provider("my-vllm", {                    -- a description, handed in
//!   name = "My vLLM box",
...
//! })
//!
//! magi.on.session(function(s) ... end)          -- behaviour, registered, repeatable
//! ```
//!
//! A provider is a **table handed to a registrar**, never a fragment the config assembles by
//! hand. That is why `magi.provider(id, spec)` takes the id separately: the registration has an
