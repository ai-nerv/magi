...
const OUTSIDE: &str = "magi.shell is only available inside a tool's run function";
...
pub fn callback<'gc>(ctx: Context<'gc>, lent: Lent, inside: Inside) -> Callback<'gc> {
    Callback::from_fn(&ctx, move |ctx, _exec, mut stack| {
        ...
        let Value::String(command) = command else {
            stack.replace(ctx, (Value::Nil, "magi.shell(command): a string"));
...
mod tests {
    ...
    impl magi_tools::Ops for Watching {
        fn cwd(&self) -> PathBuf {
            std::env::temp_dir()
        }
        ...
        fn allow(&self, _tool: &str, _action: &magi_tools::permit::Action) -> Result<(), String> {
            if self.allow {
                Ok(())
            } else {
    ...
    fn through(ops: Option<std::rc::Rc<Watching>>, body: &str) -> (String, Option<String>) {
        ...
        engine
            .run(
                &format!(
                    "magi.tool(\"probe\", {{ transport = {{ kind = \"lua\" }},\n\
    ...
    fn a_tool_can_run_a_command() {
        ...
        let (content, ran) = through(
            Some(ops),
            "local out = magi.shell('echo hi') return { content = out }",
        );
    ...
    fn a_refused_command_never_runs() {
        ...
        let (content, ran) = through(
            Some(ops),
            "local out, err = magi.shell('rm -rf /') return { content = tostring(err) }",
        );
    ...
    fn a_config_being_read_cannot_spawn() {
        ...
        engine
            .run(
                "local out, err = magi.shell('echo hi')\nmagi.answer = tostring(err)",
                "at-load.lua",
            )
            .expect("it runs");
        engine.harvest();
    ...
    fn a_non_string_argument_is_refused_rather_than_raising() {
        ...
        let (content, ran) = through(
            Some(ops),
            "local out, err = magi.shell(42) return { content = tostring(err) }",
        );
        assert_eq!(ran, None);
    ...
    fn the_sandbox_still_refuses_the_other_way_in() {
        // `magi.shell` is an addition, not a relaxation. `os.execute` stays gone.
        let mut engine = Engine::new();
        engine
            .run("magi.answer = tostring(os.execute)", "probe.lua")
            .expect("runs");
        engine.harvest();
        assert_eq!(
