//! Unix-socket transport for the axon wire contract.
//!
...
pub fn default_socket_path() -> PathBuf {
    ...
    runtime_dir.join("axon").join("host.sock")
}

/// The socket a daemon serving `cwd` listens on.
///
/// Per-directory rather than one path for the machine: `axon` in two repositories is two
...
pub fn socket_for(cwd: &Path) -> PathBuf {
    let runtime_dir = std::env::var_os("XDG_RUNTIME_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(std::env::temp_dir);
    runtime_dir.join("axon").join(format!(
        "{:016x}.sock",
...
mod tests {
    ...
    fn socket_path_lives_under_a_runtime_dir() {
        ...
        assert!(path.ends_with("axon/host.sock"), "got {}", path.display());
    }
}
