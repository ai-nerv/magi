...
use axon_host::turn::{Backend, run};
use axon_lua::adapter::{LuaAdapter, engine_with_builtins};
use axon_proto::{Entry, SessionId, StopReason};
...
fn session(name: &str) -> (tokio::sync::Mutex<Session>, PathBuf) {
    let dir = std::env::temp_dir().join(format!("axon-turn-{}-{name}", std::process::id()));
...
async fn a_turn_streams_into_the_journal() {
    ...
    let registry = axon_tools::Registry::new();
...
async fn an_interrupt_stops_a_turn_the_model_has_not_finished() {
    ...
    assert_eq!(*held.status(), axon_proto::AgentStatus::Idle);
...
async fn a_compacted_session_sends_the_summary_and_not_the_history() {
    ...
    let (session, dir) = session("compacted-context");
    {
        ...
        held.commit(Entry::Compaction {
            id: axon_proto::MessageId::new("k1"),
    ...
    let held = session.lock().await;
    let context = axon_host::context::of(&held);
...
async fn the_wait_is_announced_while_it_is_happening() {
    ...
    while let Ok(event) = live.try_recv() {
        if let axon_proto::HarnessEvent::StatusChanged {
            status:
                axon_proto::AgentStatus::Retrying {
                    attempt, delay_ms, ..
                },
            ..
        } = event
