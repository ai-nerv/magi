...
//!
//! M1's claim is that `axon host` can stand in for the replay host without the UI changing.
//! These drive the same protocol the UI drives, so the claim is tested rather than asserted.

use axon_host::{open_session, serve};
use axon_ipc::{FrameReader, FrameWriter};
use axon_proto::{Cursor, Entry, HarnessEvent, UiCommand};
use std::path::{Path, PathBuf};

fn temp(name: &str) -> (PathBuf, PathBuf) {
    let dir = std::env::temp_dir().join(format!("axon-host-{}-{name}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).expect("mkdir");
    // Short enough to stay under SUN_LEN, which a scratch directory path is not.
    let socket = std::env::temp_dir().join(format!("axon-h-{}-{name}.sock", std::process::id()));
...
impl Client {
    async fn attach(socket: &Path, from: Cursor) -> (Self, Vec<Entry>) {
        let stream = axon_ipc::connect(socket).await.expect("connect");
    ...
    async fn model_of(socket: &Path) -> Option<axon_proto::ModelInfo> {
        let stream = axon_ipc::connect(socket).await.expect("connect");
        let (read_half, write_half) = stream.into_split();
...
async fn start_with_backend(name: &str, base_url: String) -> (PathBuf, PathBuf) {
    ...
    let backend = axon_host::turn::Backend {
        apis: axon_lua::adapter::shipped_apis().expect("the checkout's protocols"),
        tools: Vec::new(),
        clients: Vec::new(),
        ...
        system: Some("You are axon.".to_owned()),
        confine: false,
        grants: Vec::new(),
    ...
    for _ in 0..10 {
        ...
        if matches!(
            ...
            HarnessEvent::AssistantEnded {
                stop_reason: axon_proto::StopReason::Aborted,
                ..
            }
        ) {
            aborted = true;
...
async fn an_amended_entry_is_not_announced_as_a_new_one() {
    ...
    // body, because a UI appending deltas would otherwise show the opening twice.
    {
        ...
        held.amend(Entry::Assistant {
            ...
            thinking: String::new(),
            stop_reason: Some(axon_proto::StopReason::EndTurn),
            error: None,
            signatures: axon_proto::Signatures::default(),
            usage: axon_proto::Usage::default(),
        })
...
fn two_models() -> axon_host::catalog::Catalog {
    let providers =
        serde_json::from_value::<Vec<axon_provider::provider::Provider>>(serde_json::json!([
            {
            ...
            },
            {
                ...
                "auth": { "kind": "api-key", "vars": ["AXON_TEST_UNSET_KEY"] },
                "models": [
...
async fn start_with_catalog(name: &str) -> (PathBuf, PathBuf) {
    ...
    let listener = axon_ipc::bind(&socket).await.expect("bind");
    tokio::spawn(
        async move { axon_host::serve_catalog(listener, session, backend, catalog).await },
    );
    (dir, socket)
}
...
async fn a_model_with_no_credential_is_refused_with_what_to_set() {
    ...
    assert!(message.contains("AXON_TEST_UNSET_KEY"), "{message}");
    let _ = std::fs::remove_dir_all(&dir);
    let _ = std::fs::remove_file(&socket);
}
