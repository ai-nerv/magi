...
impl Worker {
    ...
    pub fn gated(
        backend: Backend,
        approver: Option<std::sync::Arc<dyn axon_tools::approve::Approver>>,
    ) -> Self {
        ...
        std::thread::spawn(move || {
            ...
            if let Some(why) = broken {
                eprintln!("axon host: {why}");
                return;
            }

            let engine = std::rc::Rc::new(std::cell::RefCell::new(engine));
            let mut registry = axon_tools::Registry::new();
...
async fn declare(
    ...
    let say = |session: &Arc<Mutex<Session>>, message: String| {
        ...
        async move {
            ...
            let _ = held.publisher().send(axon_proto::HarnessEvent::Refused {
                cursor: held.cursor(),
                message,
            });
        }
    ...
    context
        .messages
        .push(axon_model::Message::user(crate::declaring::question(
            &backend.cwd,
        )));

    let options = axon_provider::api::Options {
    ...
    for need in needs {
        ...
        let action = match &grant.scope {
            axon_proto::permit::Scope::Program { program } => axon_proto::permit::Action::Run {
                command: program.clone(),
                program: program.clone(),
            },
            axon_proto::permit::Scope::Directory { path } => match need.verb.as_str() {
                "write" => axon_proto::permit::Action::Write { path: path.clone() },
                "reach" => axon_proto::permit::Action::Network { host: path.clone() },
                _ => axon_proto::permit::Action::Read { path: path.clone() },
            },
            _ => continue,
        };
...
mod tests {
    use super::*;
    use axon_proto::{Entry, SessionId};
    ...
    async fn a_dropped_worker_does_not_strand_its_caller() {
        ...
        let dir = std::env::temp_dir().join(format!("axon-worker-{}", std::process::id()));
    ...
    async fn a_session_is_usable_after_a_worker_refuses() {
        ...
        let dir = std::env::temp_dir().join(format!("axon-worker2-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let session =
            Session::open(&dir.join("s.jsonl"), SessionId::new("s"), "/tmp", 0).expect("session");
        let session = Arc::new(Mutex::new(session));
