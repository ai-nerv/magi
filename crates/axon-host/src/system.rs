...
//! are configuration and belong to whoever installed axon. The facts — which directory, which
...
/// convention worse, and a project that wants axon to read something else can `include` it
/// from this.
...
mod tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("axon-system-{}-{name}", std::process::id()));
