...
use axon_ipc::{FrameReader, FrameWriter, IpcError, PeerCred};
...
pub async fn serve_catalog(
    ...
    let approver: Arc<dyn axon_tools::approve::Approver> = {
...
async fn connection(
    ...
    approver: &Arc<dyn axon_tools::approve::Approver>,
) -> Result<(), HostError> {
...
async fn switch_thinking(
    ...
    let Ok(parsed) = serde_json::from_value::<axon_model::ThinkingLevel>(
...
pub fn no_model(catalog: &crate::catalog::Catalog) -> String {
    ...
    if ready.is_empty() {
        said.push_str(" Nothing else is ready either — set a provider key, or run `axon models` to see what each one needs.");
    } else {
        ...
        said.push_str(&format!(
            " Ready now: {}{tail}. Type `/model` to switch, or set `axon.model` in your config.",
            ready[..shown].join(", ")
        ));
    }
    said
...
mod no_model_tests {
    ...
    fn a_configured_model_with_no_key_is_not_called_unconfigured() {
        ...
        // `axon.model` was set and whose key merely was not. Two different problems, one
        // sentence, and the sentence sent people to the wrong one.
        let said = super::no_model(&wanting("paid/x"));
        assert!(said.contains("AXON_TEST_NOT_SET"), "{said}");
