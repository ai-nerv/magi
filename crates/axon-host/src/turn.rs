...
//! agent logic — [`axon_core::Turn`] decides what happens and this drives it.

use crate::session::Session;
use axon_core::{Step, Turn};
use axon_model::StopReason;
use axon_proto::{AgentStatus, Entry, MessageId, ToolCallId};
use axon_provider::api::Options;
use axon_provider::client::Client;
use axon_provider::model::Model;
use axon_provider::provider::Provider;
use axon_tools::{Ops, Registry};
...
pub struct Backend {
    ...
    ///
    /// Off unless a config asks. See [`axon_tools::ops::Real`] for why a wall only the careful
    ...
    /// edited `apis/*.lua` changed what `axon models` reported and nothing else.
...
async fn compact(
    ...
    let mut turn = axon_core::Turn::new();
    let mut deltas = Vec::new();
    let outcome = client
        .stream(
            &axon_provider::client::Call {
                adapter,
...
async fn one_turn(
    ...
    let outcome = {
        ...
        let call = axon_provider::client::Call {
            adapter,
            provider: &backend.provider,
            model: &backend.model,
            context: &context,
    ...
    if cancel.is_requested() {
        ...
        held.amend(Entry::Assistant {
            ...
            error: None,
            signatures: axon_proto::Signatures {
                text: None,
    ...
    if !matches!(turn.state(), axon_core::TurnState::ToolsPending) {
        held.set_status(AgentStatus::Idle);
    }
...
struct Round {
    ...
    failed: Option<axon_provider::retry::RetryClass>,
}
...
fn assistant(id: &MessageId, turn: &Turn) -> Entry {
    Entry::Assistant {
        ...
        stop_reason: match turn.state() {
            axon_core::TurnState::Finished(reason) => Some(reason),
            ...
            axon_core::TurnState::ToolsPending => Some(StopReason::ToolUse),
            _ => None,
        },
        error: None,
    }
...
pub async fn run(
    ...
    for _ in 0..MAX_ROUNDS {
        ...
        if round.failed == Some(axon_provider::retry::RetryClass::Overflow) && !compacted {
            compacted = true;
        ...
        for ((call, prepared), at) in calls.iter().zip(prepared).zip(at) {
            ...
            let output = match prepared {
                ...
                _ => axon_tools::Output::error("cancelled before this tool ran"),
            };
            let mut held = session.lock().await;
            held.amend_at(
                at,
...
fn turn_calls(turn: &Turn) -> Vec<axon_core::PendingCall> {
    match turn.step() {
