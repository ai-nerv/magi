...
use axon_proto::permit::{Action, Decision};
use axon_proto::{Cursor, HarnessEvent, ToolCallId};
...
impl axon_tools::approve::Approver for Asker {
    fn ask(&self, tool: &str, action: &Action) -> Decision {
        ...
        (self.publish)(HarnessEvent::PermissionAsked {
            ...
            tool: tool.to_owned(),
            action: action.clone(),
            offers: axon_tools::permit::Ledger::offers(action),
        });
...
mod tests {
    use super::*;
    use axon_tools::approve::Approver;
        ...
        let answering = std::thread::spawn(move || {
            ...
            for _ in 0..200 {
                ...
                pending.answer(
                    &id,
                    Decision::Allow {
                        scope: axon_proto::permit::Scope::Once,
