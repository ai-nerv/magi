...
mod tests {
    ...
    fn sessions_live_under_a_data_directory() {
        assert!(sessions_dir().ends_with("axon/sessions"));
    }
...
fn summary(path: &std::path::Path) -> Option<Summary> {
    ...
    let recovered = axon_journal::parse(&text).ok()?;
    let title = recovered
        .entries
        .iter()
        .find_map(|entry| match entry {
            axon_proto::Entry::User { text, .. } => Some(text.clone()),
            _ => None,
        })
        .unwrap_or_default();
    Some(Summary {
