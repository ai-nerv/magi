...
use axon_journal::{Journal, JournalError};
use axon_proto::{AgentStatus, Cursor, Entry, HarnessEvent, SessionId};
use std::path::Path;
...
pub struct Session {
    ...
    choices: Vec<axon_proto::ModelChoice>,
    ...
    model: Option<axon_proto::ModelInfo>,
    journal: Journal,
    status: AgentStatus,
...
impl Session {
    ...
    pub fn set_model(&mut self, model: Option<axon_proto::ModelInfo>) {
        self.model = model;
    }

    /// Say what it could switch to.
    pub fn set_choices(&mut self, choices: Vec<axon_proto::ModelChoice>) {
    ...
    pub fn usage(&self) -> axon_proto::Usage {
        self.entries()
            ...
            .fold(axon_proto::Usage::default(), |total, entry| match entry {
                Entry::Assistant { usage, .. } => axon_proto::Usage {
...
mod tests {
    ...
    fn temp(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("axon-session-{}-{name}", std::process::id()));
...
mod streaming_tests {
    use super::*;
    use axon_proto::{MessageId, Signatures, StopReason, Usage};

    fn journal_path(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("axon-stream-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        dir.join("s.jsonl")
    }
