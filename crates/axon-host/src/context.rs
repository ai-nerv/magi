...
use axon_model::{Content, Context, Message, Role, StopReason};
...
pub fn of(session: &Session) -> Context {
    ...
    for entry in live {
        match entry {
            ...
            // provider would be telling the model what axon told somebody about axon.
...
/// [`axon_provider::retry`] classifies `Invalid` — neither retryable nor `Overflow` — so nothing
...
mod context_tests {
    ...
    fn session(name: &str) -> (Session, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("axon-ctx-{}-{name}", std::process::id()));
...
mod branch_tests {
    use super::*;
    use axon_proto::{MessageId, SessionId, Signatures};

    fn session(name: &str) -> (Session, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("axon-branch-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let session =
            Session::open(&dir.join("s.jsonl"), SessionId::new("s"), "/tmp", 0).expect("session");
        (session, dir)
