...
fn a_setting_is_assigned_not_declared_in_a_table() {
    let config = run(r#"axum.model = "anthropic/claude-sonnet-4-5""#);
...
fn nested_data_stays_nested() {
    ...
    let config = run(r#"
        axum.keys = { submit = "enter", newline = "shift+enter" }
        "#);
...
fn a_config_can_read_back_what_it_assigned() {
    let config = run(r#"
        axum.model = "a"
        if axum.model == "a" then axum.model = "b" end
        "#);
    assert_eq!(
        config.string("model"),
        Some("b"),
...
fn the_config_returns_nothing_and_may_branch_and_loop() {
    ...
    let config = run(r#"
        for _, id in ipairs({ "a", "b", "c" }) do
          axum.provider(id, { name = id, api = "openai-completions" })
        end
        "#);
...
fn registration_is_keyed_so_re_running_replaces_rather_than_appends() {
    ...
    let config = run(r#"
        axum.provider("box", { name = "first" })
        axum.provider("box", { name = "second" })
        "#);
...
fn each_registrar_keeps_its_own_namespace() {
    let config = run(r#"
        axum.provider("same", { which = "provider" })
        axum.agent("same", { which = "agent" })
        "#);
...
fn a_registrar_without_a_name_is_refused() {
    let mut engine = Engine::new();
    assert!(engine.run("axum.provider({})", "bad.lua").is_err());
}
...
fn a_config_may_probe_and_pcall_its_own_mistakes() {
    ...
    let config = run(r#"
        local ok = pcall(function() axum.provider({}) end)
        axum.probed = ok
        axum.provider("good", {})
        "#);
...
fn later_files_win_over_earlier_ones() {
    let mut engine = Engine::new();
    engine
        .run("axum.model = 'machine'", "init.lua")
        .expect("run");
    engine
        .run("axum.model = 'project'", ".axum.lua")
        .expect("run");
    engine.harvest();
...
fn real_lua_is_available_not_a_subset() {
    ...
    let config = run(r#"
        ...
        axum.model = join("anthropic", "claude"):gsub("claude", "sonnet")
        "#);
...
fn a_project_config_is_applied_after_a_machine_one() {
    ...
    assert!(paths.last().expect("a path").ends_with(".axum.lua"));
}
