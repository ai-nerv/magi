...
pub enum Auth {
    ...
    OAuth {
        ...
        /// a sign-in that fails for a reason nobody can act on. Absent, `axon auth login`
...
mod tests {
    ...
    fn a_provider_without_its_variable_set_is_not_configured() {
        let p = declared(serde_json::json!({
            ...
            "auth": { "kind": "api-key", "vars": ["AXON_TEST_KEY_DEFINITELY_UNSET"] },
            "models": [{ "id": "m", "name": "M", "context_window": 8192, "max_tokens": 4096 }]
        }));
        assert!(!p.is_configured());
        assert_eq!(p.auth.requirement(), "set AXON_TEST_KEY_DEFINITELY_UNSET");
    }
