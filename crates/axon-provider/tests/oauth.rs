...
//!
//! The browser leg cannot be automated, so the test *is* the browser: it reads the URL axon
...
use axon_provider::oauth::{Pkce, Store, Tokens, authorize_url, exchange, listen_for_code};
...
async fn exchanging_a_code_sends_the_verifier_and_stores_what_comes_back() {
    ...
    assert!(tokens.expires_at > axon_provider::oauth::now());
}
...
async fn a_reply_with_no_expiry_is_given_a_short_one() {
    ...
    let tokens = exchange(&reqwest::Client::new(), &url, &[])
        .await
        .expect("tokens");
    let life = tokens.expires_at - axon_provider::oauth::now();
...
fn the_store_is_written_readable_only_by_its_owner() {
    ...
    let dir = std::env::temp_dir().join(format!("axon-oauth-{}", std::process::id()));
