-- axon's client library: what another program requires to talk to a running agent.
--
--   local axon = load(src)(my_transport)
--   local agent = axon.connect()
...
-- `io.popen("axon lua-api"):read("a")` where the host permits `io`.
--
-- Plain Lua on purpose. It runs unchanged in axon's own VM, in hexe's, in oslo's, in PUC Lua and in whatever
...
--
-- Inside axon that is `axon.stream`, and it is found automatically when nothing is passed.
--
...
-- Where the socket primitive comes from when the caller did not say. Inside axon the whole library
...
local HOSTS = { "axon", "hexe", "oslo" }

-- ---------------------------------------------------------------- JSON, in Lua

-- Carried rather than required. The library runs inside somebody else's VM, so it cannot reach
-- axon's own JSON — and a client that depended on the host having a JSON module would be a client
...
function encode(v, depth)
  depth = (depth or 0) + 1
  if depth > 24 then error("axon: value nested too deeply to send", 0) end
  ...
  if kind == "number" then
    if v ~= v or v == math.huge or v == -math.huge then
      error("axon: " .. tostring(v) .. " cannot be sent", 0)
    end
    return (math.type and math.type(v) == "integer") and string.format("%d", v) or tostring(v)
  end
  if kind ~= "table" then error("axon: cannot send a " .. kind, 0) end
  ...
  for key, value in pairs(v) do
    if type(key) ~= "string" then error("axon: only string keys can be sent", 0) end
...
local function decode(s)
  ...
  local function fail(why)
    error("axon: bad reply at " .. at .. ": " .. why, 0)
  end
...
---
--- axon serves one request per connection, so this reconnects each time. That is the server's
...
---
--- `$AXON_API_SOCKET` first, because a program axon started inherits it and means *that* session —
...
--- looks for any of the family's globals rather than only its own. Inside oslo `_G.axon` does not
...
--- The directory axon binds its control sockets in.
---
--- Mirrors axon's own socket directory: the default session binds straight under `<runtime>/axon`,
...
local function socket_dir()
  ...
  local base = (runtime and runtime ~= "")
    and (runtime .. "/axon")
    or ("/tmp/axon-" .. (os.getenv("UID") or "0"))
...
local function find(where)
  ...
  local env = os.getenv("AXON_API_SOCKET")
...
--- Open a connection to a running axon.
---
...
function M.connect(where)
  ...
  if not candidates or #candidates == 0 then
    return nil, "no axon socket found - is a daemon running?"
  end
  ...
  -- socket timeout, which says nothing about the cause. Inside axon, `ctx.*`
  -- already reaches everything this library does, without a socket.
  local self_socket = _G.axon and _G.axon.__self_socket
  local last
...
--- narrowing is the listener's, decided when axon bound it.
---
--- Grants nothing to the caller it did not already have -- it IS the process in that pane -- but it
--- lets it ask axon precisely instead of parsing its own terminal, and it means a program handed
--- this path can be trusted with it.
---
--- `$AXON_SESSION_API_SOCKET` is exported into every pane's shell. Nothing is listening on it while no
--- frontend is attached, which is the truth about a live API rather than a fault to hide.
function M.connect_pane(opts)
  local path = os.getenv("AXON_SESSION_API_SOCKET")
  if not path or path == "" then
    return nil, "not inside an axon session ($AXON_SESSION_API_SOCKET is unset)"
  end
