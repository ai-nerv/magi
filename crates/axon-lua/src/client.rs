//! The client library other programs load to talk to a running axon.
//!
//! Plain Lua, and *copied* from the family rather than written: oslo ships `client.lua`, hexe
//! ships `hexe.lua`, and this is the same file with axon's identity and axon's verbs. The
...
/// The client library, as a program would receive it from `axon lua-api`.
pub const CLIENT: &str = include_str!("../lua/axon.lua");
...
mod tests {
    ...
    /// Load the client in axon's own VM and ask it something.
    fn probe(script: &str) -> String {
        let mut engine = Engine::new();
        let source = format!(
            r#"
            local chunk = assert(load({CLIENT:?}, "axon.lua"))
            local axon_client = chunk(nil)
            axon.answer = tostring({script})
            "#
        );
        engine
    ...
    #[test]
    fn the_stub_loads_in_axons_own_vm() {
        ...
        assert_eq!(probe("axon_client._NAME"), "axon");
    }

    #[test]
    fn the_stub_declares_a_protocol_version() {
        assert_eq!(probe("axon_client._VERSION"), "1");
    }
    ...
    fn the_stub_offers_connect_and_fetch() {
        ...
        assert_eq!(probe("type(axon_client.connect)"), "function");
        assert_eq!(probe("type(axon_client.fetch)"), "function");
    }

    #[test]
    fn connecting_without_a_transport_fails_rather_than_hanging() {
        assert_eq!(probe("select(2, axon_client.connect()) ~= nil"), "true");
    }
