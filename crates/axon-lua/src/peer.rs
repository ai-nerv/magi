...
//!
//! axon is a client here, not a server. It loads the sibling's own client — oslo's `client.lua`,
...
//!
//! This is the direction that makes the family useful to axon. Spawning an agent in a shell is
//! oslo's job and putting it in a tab is hexe's; axon asks them to, rather than reimplementing
//! either.

use crate::{Engine, LuaError};

/// A sibling axon can talk to.
...
/// existed when it was written — inside axon, hexe's client finds neither `_G.hexe` nor
/// `_G.oslo` and falls through to shelling out. Handing it a path sidesteps a question it
/// should not have to answer, and axon knows which session it means anyway.
#[must_use]
...
/// discovery looks for its own globals first, and inside axon those are not there.
pub fn call(
    engine: &mut Engine,
    client: &str,
    verb: &str,
    ...
    let source = format!(
        ...
        if not peer then
          axon.answer = "no: " .. tostring(why)
          return
        end
        local ok, result = pcall(function() return peer.{verb}() end)
        peer:close()
        if not ok then
          axon.answer = "refused: " .. tostring(result)
          return
        end
        ...
        if type(result) == "table" then
          ...
          axon.answer = table.concat(names, " ")
        else
          axon.answer = tostring(result)
        end
        "#
    );
...
mod tests {
    ...
    #[test]
    fn a_sibling_stub_loads_in_axons_vm() {
        ...
        for sibling in SIBLINGS {
            ...
            engine
                .run(&source, "load.lua")
                .unwrap_or_else(|e| panic!("{}'s client must load in axon: {e}", sibling.name));
            engine.harvest();
    ...
    fn the_stream_primitive_reports_a_missing_socket_rather_than_hanging() {
        ...
        engine
            .run(
                r#"
                local h, why = __stream.connect("/nonexistent/axon-test.sock", 200)
                axon.answer = tostring(h) .. "|" .. tostring(why)
                "#,
                "missing.lua",
            )
            .expect("run");
    ...
    fn a_handle_round_trips_bytes() {
        ...
        let path = std::env::temp_dir().join(format!("axon-stream-{}.sock", std::process::id()));
