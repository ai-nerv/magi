...
//! discover their own sockets while running inside axon.
//!
//! **`fs.dir` is deliberately not offered.** A client asks the host for "the directory my
//! sockets live in", and any host that answers gets believed — so axon answering would send
//! hexe's client looking for hexe sockets in axon's directory. Listing is generic and safe to
