//! The Lua VM axon runs on, and the config API it offers `init.lua`.
//!
...
//!
//! axon.model = "anthropic/claude-sonnet-4-5"   -- a setting, assigned
//! axon.scan_speed = 2                          -- another one
//!
//! axon.provider("my-vllm", {                    -- a description, handed in
//!   name = "My vLLM box",
...
//! })
//!
//! axon.on.session(function(s) ... end)          -- behaviour, registered, repeatable
//! ```
//!
//! A provider is a **table handed to a registrar**, never a fragment the config assembles by
//! hand. That is why `axon.provider(id, spec)` takes the id separately: the registration has an
...
pub fn search_paths() -> Vec<std::path::PathBuf> {
    ...
    {
        out.push(config.join("axon").join("init.lua"));
    }
    out.push(std::path::PathBuf::from(".axon.lua"));
    out
}
