...
use super::support::*;
use super::*;
use axon_model::{Content, Message, Role, StopReason, ThinkingLevel};
...
fn completions_honours_a_declared_dialect() {
    ...
    m.compat = Some(axon_provider::compat::Compat {
        max_tokens_field: Some(axon_provider::compat::MaxTokensField::MaxCompletionTokens),
        supports_developer_role: Some(true),
        ..axon_provider::compat::Compat::default()
    });
    let context = Context {
...
mod schema_tests {
    ...
    fn asking_for_a_shape() -> Options {
        Options {
            schema: Some(axon_provider::api::Schema {
                name: "answer".into(),
