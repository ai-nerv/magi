...
//!
//! The only implementation of [`Adapter`] there is. `axon-provider` owns the contract and moves
...
use axon_provider::api::{Adapter, Delta, Options, StreamState};
use axon_provider::model::Model;
use axon_provider::sse;
...
impl LuaAdapter {
    ...
    pub fn from_shared(
        ...
        if !known.iter().any(|a| a == name) {
            return Err(format!(
                "no protocol named {name:?} is registered; axon knows {}",
                if known.is_empty() {
                    "none".to_owned()
                } else {
                    known.join(", ")
...
/// lives in `axon_provider::compat::Resolved` and nowhere else — ten descriptions each
...
fn described(model: &Model) -> serde_json::Value {
    ...
    if let Some(object) = value.as_object_mut() {
        let resolved = axon_provider::compat::resolve(model.compat);
        object.insert(
            "compat".to_owned(),
...
fn effective_thinking(model: &Model, options: &Options) -> Option<String> {
    ...
    if level == axon_model::ThinkingLevel::Off {
        return None;
    }
...
/// Protocols axon knows of but does not speak, and why.
///
