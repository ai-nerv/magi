...
//!
//! Everything a person chose *while using* magi used to be forgotten the moment the daemon
//! stopped. `/model` switched to a model and the next run went back to whatever `magi.model`
...
//! hand-editing — `magi.model` is still where a chosen default is *written down*, and this only
...
pub struct Chosen {
    /// The model, as `magi models` names it.
...
mod tests {
    ...
    fn the_record_sits_beside_the_journals_rather_than_in_the_config() {
        // Losing it costs a preference, not a setting. `magi.model` is still where a chosen
