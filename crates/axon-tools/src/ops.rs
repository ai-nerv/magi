...
pub trait Ops: Send + Sync {
    ...
    fn allow(&self, tool: &str, action: &axon_proto::permit::Action) -> Result<(), String> {
        let _ = (tool, action);
        Ok(())
    }
...
///
/// Confinement is a *configuration*, the same way sandboxing is (§5e): `axon.confine = true`
...
impl Real {
    ...
    #[must_use]
    pub fn grants(&self) -> Vec<axon_proto::permit::Grant> {
    ...
    fn resolve(&self, path: &Path) -> Result<PathBuf, String> {
        ...
        if !normalised.starts_with(&root) {
            return Err(format!(
                "{} is outside this session's directory, and `axon.confine` is on",
                path.display()
            ));
        }
        Ok(normalised)
...
impl Ops for Real {
    ...
    fn allow(&self, tool: &str, action: &axon_proto::permit::Action) -> Result<(), String> {
        ...
        match decision {
            axon_proto::permit::Decision::Allow { .. } => Ok(()),
            axon_proto::permit::Decision::Deny => Err(format!(
...
mod tests {
    ...
    fn rooted(name: &str) -> (Real, PathBuf) {
        let dir = std::env::temp_dir().join(format!("axon-ops-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("mkdir");
        (Real::new(dir.clone()), dir)
    }

    /// The same, with the wall on: `axon.confine` is where that rule lives now.
    fn walled(name: &str) -> (Real, PathBuf) {
        let dir = std::env::temp_dir().join(format!("axon-wall-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("mkdir");
    ...
    fn a_command_runs_in_the_session_directory() {
        ...
        assert!(result.stdout.contains("axon-ops-"), "{}", result.stdout);
        let _ = std::fs::remove_dir_all(&dir);
    }
...
mod reach_tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("axon-reach-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("mkdir");
        dir
    }
    ...
    fn confined_ops_still_refuse_and_say_why() {
        ...
        let outside = std::env::temp_dir().join("axon-not-here.txt");
        let why = ops.read(&outside).expect_err("refused");
        assert!(why.contains("axon.confine"), "it names the setting: {why}");
        let _ = std::fs::remove_dir_all(&session);
    }
...
mod gate_tests {
    use super::*;
    use axon_proto::permit::{Action, Decision, Lifetime, Scope};
    use std::sync::Arc;
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("axon-gate-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("mkdir");
        dir
    }
