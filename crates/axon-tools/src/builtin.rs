...
impl Tool for Write {
    ...
    fn run(&self, arguments: &Value, ops: &dyn Ops, _cancel: &dyn Cancel) -> Output {
        ...
        if let Err(why) = ops.allow(
            "write",
            &axon_proto::permit::Action::Write {
                path: ops.cwd().join(path).display().to_string(),
            },
        ) {
            return Output::error(why);
...
mod tests {
    ...
    fn session(name: &str) -> (Registry, Real, PathBuf) {
        let dir = std::env::temp_dir().join(format!("axon-builtin-{}-{name}", std::process::id()));
    ...
    fn a_confined_session_refuses_an_escape_from_every_tool() {
        // The rule lives on `axon.confine` now, and this is what it buys when it is on.
        let dir = std::env::temp_dir().join(format!("axon-bwall-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("mkdir");
...
mod paging_tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("axon-read-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("mkdir");
        dir
    }
