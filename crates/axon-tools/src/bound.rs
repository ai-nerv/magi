...
fn spill(tool: &str, content: &str) -> Option<PathBuf> {
    let dir = std::env::temp_dir().join("axon-output");
...
mod tests {
    ...
    fn the_note_says_how_much_went_and_where() {
        ...
        assert!(note.contains("/axon-output/"), "it names the spill: {note}");
    }
...
mod expiry_tests {
    ...
    fn a_stale_spill_is_dropped_on_the_way_past() {
        ...
        // produced it is gone.
        let dir = std::env::temp_dir().join(format!("axon-expire-{}", std::process::id()));
    ...
    fn a_fresh_spill_is_left_alone() {
        let dir = std::env::temp_dir().join(format!("axon-fresh-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("mkdir");
    ...
    fn expiring_a_directory_that_is_not_there_is_not_an_error() {
        expire(&std::env::temp_dir().join("axon-does-not-exist-at-all"));
    }
}
