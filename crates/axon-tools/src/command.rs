...
//! transport wants a peer speaking axon's framed protocol, so `command = "rg"` was a broken pipe
...
impl CommandTool {
    ...
    /// Environment beside what every process axon starts already gets.
    #[must_use]
    ...
    fn action(&self, rendered: &[String]) -> axon_proto::permit::Action {
        let mut shown = vec![self.program.clone()];
        shown.extend(rendered.iter().cloned());
        axon_proto::permit::Action::Run {
            command: shown.join(" "),
...
mod running_tests {
    ...
    impl Ops for Refusing {
        ...
        fn allow(&self, _tool: &str, _action: &axon_proto::permit::Action) -> Result<(), String> {
    ...
    fn a_refused_call_never_runs_the_program() {
        let dir = std::env::temp_dir().join(format!("axon-refused-{}", std::process::id()));
    ...
    fn a_program_that_is_not_there_is_reported_not_panicked() {
        ...
        let tool = CommandTool::new(
            ...
            "",
            json!({ "type": "object" }),
            "axon-no-such-program-anywhere",
            vec![],
        );
        let out = tool.run(&json!({}), &ops, &Uncancelled);
        assert!(out.is_error);
        assert!(
            out.content.contains("axon-no-such-program-anywhere"),
            "{}",
            out.content
        );
    }
