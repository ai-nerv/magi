...
//!
//! **What is stored, and where.** `$XDG_DATA_HOME/axum/credentials.json`, mode `0600`, one
...
pub fn path() -> PathBuf {
    ...
    base.join("axum").join("credentials.json")
}
...
pub enum Error {
    ...
    #[error("not signed in to {0}; run `axum auth login {0}`")]
    NotSignedIn(String),

    /// The provider issued no refresh token, and the access token has expired.
    #[error("the session for {0} expired; run `axum auth login {0}` again")]
    Expired(String),
}
...
mod tests {
    ...
    fn credentials_live_beside_the_sessions_not_the_config() {
        ...
        let path = path();
        assert!(
            path.ends_with("axum/credentials.json"),
            "{}",
            path.display()
        );
        assert!(
