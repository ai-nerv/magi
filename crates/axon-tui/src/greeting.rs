...
//!
//! Opening axon in a fresh directory drew twenty-five blank rows and an empty box. There was
...
pub fn render(model: &str, cwd: &str, width: u16) -> Vec<Line<'static>> {
    ...
    if !model.is_empty() && model != crate::glyph::no_model() {
...
mod tests {
    ...
    fn it_says_what_it_is_and_what_it_will_use() {
        ...
        assert!(text.contains("axon"), "{text}");
