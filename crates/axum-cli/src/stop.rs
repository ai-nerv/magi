//! `axum stop` — ending a daemon on purpose.
//!
...
//! per project still holding a socket. `ps | grep axum` is not an answer a tool should expect
...
pub fn run(socket: &Path, all: bool) -> Result<()> {
    ...
    for pid_file in targets {
        match stop_one(&pid_file) {
            ...
            Stopped::NotRunning => {}
            Stopped::Stuck(pid) => {
                eprintln!("axum: {pid} did not stop; it may be mid-turn");
            }
        }
    }
...
fn running() -> Vec<PathBuf> {
    let Some(dir) = axum_ipc::default_socket_path()
        .parent()
...
mod tests {
    ...
    fn a_pid_file_with_nothing_in_it_is_not_running() {
        let dir = std::env::temp_dir().join(format!("axum-stop-{}", std::process::id()));
    ...
    fn a_pid_file_for_a_process_that_is_gone_is_cleaned_up() {
        ...
        let dir = std::env::temp_dir().join(format!("axum-stop-gone-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("mkdir");
        let path = dir.join("host.pid");
    ...
    fn a_missing_pid_file_is_not_an_error() {
        let path = std::env::temp_dir().join("axum-stop-nothing-here.pid");
