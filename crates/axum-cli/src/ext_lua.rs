//! `axum ext lua` — a second peer, so the protocol has more than one implementation.
//!
...
use axum_proto::{ToolReport, ToolRequest};
use std::path::Path;
...
mod tests {
    ...
    fn peer_file(name: &str, source: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("axum-extlua-{}", std::process::id()));
