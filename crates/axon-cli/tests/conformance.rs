...
//!
//! Including one that shares no code with axon at all: `tests/fixtures/echo.c` was written
...
use axon_testkit::conformance::{Subject, check};
use std::path::{Path, PathBuf};

fn scratch(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("axon-conf-{}-{name}", std::process::id()));
...
fn the_shell_peer_conforms() {
    ...
    assert_conforms(&Subject {
        command: env!("CARGO_BIN_EXE_axon"),
...
#[test]
fn a_peer_that_shares_no_code_with_axon_conforms() {
