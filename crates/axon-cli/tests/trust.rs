...
//!
//! `.axon.lua` arrives with a checkout. Cloning something and running `axon` in it must not be
...
fn workspace(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("axon-trust-{}-{name}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    install_config(&dir.join("config/axon"));
    std::fs::create_dir_all(dir.join("config/axon/tools")).expect("mkdir");
...
fn machine(dir: &Path, file: &str, source: &str) {
    let path = dir.join("config/axon").join(file);
    if file == "init.lua" {
...
fn project(dir: &Path, source: &str) {
    std::fs::write(dir.join("project/.axon.lua"), source).expect("write");
}
...
fn the_machine_config_can_add_a_tool_that_a_project_cannot() {
    ...
    std::fs::write(
        dir.join("config/axon/tools/mine.lua"),
        r#"
axon.tool("mine", {
...
"#,
    ...
    machine(&dir, "init.lua", "axon.load(\"tools/mine.lua\")\n");
...
fn the_installed_tool_file_is_the_one_that_runs() {
    ...
    // which file was read. This test passed for a year of afternoons because the `axon` it
...
fn no_two_shipped_tool_files_claim_the_same_tool() {
    ...
    for entry in std::fs::read_dir(&dir).expect("config").flatten() {
        ...
        for line in source.lines() {
            if let Some(rest) = line.trim().strip_prefix("axon.tool(\"")
