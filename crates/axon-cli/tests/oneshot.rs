//! `axon -p` and `axon --resume`, driving the real binary.
//!
...
//! person invoking `axon` gets. The properties under test only exist at this level. That a
...
/// `axon -p` against the same session is a second request to the same provider.
...
///
/// The provider goes in the machine's own configuration rather than a project `.axon.lua`,
...
fn workspace(name: &str, base_url: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("axon-one-{}-{name}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(dir.join("run")).expect("mkdir");
    std::fs::create_dir_all(dir.join("sessions")).expect("mkdir");
    install_config(&dir.join("config/axon"));
    std::fs::write(
        dir.join("config/axon/providers.lua"),
        format!(
            "axon.provider(\"fake\", {{\n\
             \x20 name = \"Fake\",\n\
...
fn a_one_shot_leaves_a_session_behind() {
    ...
    let output = axon(&dir, &["--sessions", "sessions", "-p", "say something"]);
...
fn resuming_reconstructs_the_context_the_model_is_given() {
    ...
    assert!(
        axon(&dir, &["--sessions", "sessions", "-p", "remember gerbil"])
            .status
            .success()
    );
...
fn a_daemon_killed_mid_turn_leaves_a_journal_that_still_loads() {
    ...
    let reopened = axon_journal::Journal::open(
        &journal,
        axon_proto::SessionId::new("unused"),
    ...
    assert!(
        reopened
            .entries()
            .iter()
            .any(|e| matches!(e, axon_proto::Entry::User { text, .. }
...
fn print_mode_waits_for_the_answer_after_a_tool_runs() {
    ...
    let output = axon(&dir, &["--sessions", "sessions", "-p", "count the lines"]);
