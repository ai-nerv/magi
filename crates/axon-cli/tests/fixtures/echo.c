/* A tool peer written against the documentation, not against axon's code.
 *
 ...
 * shares nothing: no axon crate, no CBOR library, no Rust. Every byte below was produced from
 ...
 * Check:  it is run by `crates/axon-cli/tests/conformance.rs`, which is the point of it.
 *
...
static void declare(void) {
    ...
    text("echo");
    text("description");
    text("Repeat what it is given, from a peer that shares no code with axon.");
    text("parameters");
    map(3);
    text("type");
    text("object");
