//! `axon ext shell` — the peer that makes `bash` work.
//!
...
/// Which shell to run.
///
/// `$AXON_SHELL`, then `$SHELL`, then `sh`. It was `sh` outright, which threw away the thing
...
pub fn shell_command() -> String {
    let axon_shell = std::env::var("AXON_SHELL").ok();
    let login = std::env::var("SHELL").ok();
    shell_command_from(login.as_deref(), axon_shell.as_deref())
}
...
fn make_fifo() -> anyhow::Result<std::path::PathBuf> {
    sweep_stale_fifos();
    let name = format!("axon-shell-{}-{}", std::process::id(), nonce());
...
fn spawn_shell(
    ...
    // `axon` profile's history and that is the log of what the agent did. Three lines of
    ...
    writeln!(
        to_shell,
        "__axon() {{ printf '\\n__axon_{nonce}_%s__o\\n' \"$2\" >&{REPORT_FD}; \
         {{ eval \"$1\" ; __axon_status=$? ; }} < /dev/null >&{REPORT_FD} 2>&{REPORT_FD}; \
         printf '\\n__axon_{nonce}_%s__c%s\\n' \"$2\" \"$__axon_status\" >&{REPORT_FD}; }}"
    )?;
    to_shell.flush()?;
...
impl Session {
    ...
    fn run(&mut self, command: &str) -> (String, bool) {
        ...
        let script = format!("__axon '{quoted}' {}\n", self.seq);
...
mod tests {
    ...
    fn state_persists_between_calls() {
        ...
        shell.run("export AXON_TEST_VAR=carried");
        let (output, _) = shell.run("echo $AXON_TEST_VAR");
    ...
    fn a_command_cannot_counterfeit_the_end_of_its_own_output() {
        ...
        let mut shell = Session::start().expect("a shell");
        let (output, failed) = shell.run("echo '__axon_done__0'; echo after");
