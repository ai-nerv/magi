//! `axon auth` — signing in to a subscription.
//!
...
//! and this is it.
//!
//! The browser is opened rather than embedded. axon has no business rendering a sign-in page,
...
use axon_provider::oauth::{self, Pkce, Store};
...
pub fn status() -> Result<()> {
    ...
    if !any {
        println!("No provider in your configuration signs in with `axon auth`.");
    }
    Ok(())
}
...
fn find(id: &str) -> Result<Provider> {
    ...
    loaded
        ...
        .find(|p| p.id == id)
        .with_context(|| {
            format!("there is no provider called {id:?}; `axon models --all` lists them")
        })
}
