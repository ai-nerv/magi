//! `axon -p "…"` — one prompt, one answer, no terminal.
//!
...
//! `-p` run leaves a session behind that `axon --resume` picks up, and there is one
//! implementation of the loop rather than two that agree until they stop agreeing.

use anyhow::Result;
use axon_ipc::{FrameReader, FrameWriter};

use axon_proto::{AgentStatus, Cursor, HarnessEvent, StopReason, UiCommand};
use std::path::Path;
...
pub async fn run(socket: &Path, prompt: String) -> Result<Outcome> {
    let stream = axon_ipc::connect(socket).await?;
