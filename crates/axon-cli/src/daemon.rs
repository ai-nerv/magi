...
/// Start `axon host` in the background.
///
...
mod tests {
    ...
    fn socket(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("axon-daemon-{}", std::process::id()));
...
mod startup_tests {
    ...
    fn the_log_sits_beside_the_socket() {
        let path = std::env::temp_dir().join("axon-log-test.sock");
    ...
    fn what_the_daemon_said_is_its_last_real_line() {
        let dir = std::env::temp_dir().join(format!("axon-said-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn no_log_is_no_quote_rather_than_an_empty_one() {
        let socket = std::env::temp_dir().join("axon-no-log-at-all.sock");
...
mod startup_more_tests {
    ...
    async fn a_live_daemon_is_not_replaced() {
        ...
        let dir = std::env::temp_dir().join(format!("axon-own-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
