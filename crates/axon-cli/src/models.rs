//! `axon models` — what axon can talk to, and what it would take to enable it.
...
pub fn print(all: bool) {
    ...
    if !all && configured < providers.len() {
        println!(
            "\n{} of {} providers configured. `axon models --all` shows the rest.",
            configured,
            providers.len()
        );
    }
...
fn print_provider(provider: &Provider, default: Option<&str>) {
    // A protocol axon cannot speak matters more than a missing key: no amount of configuring
    // will help, and finding that out after setting one is a waste of the reader's time.
    let status = if let Some(why) = axon_lua::adapter::why_unspoken(provider.api.as_str()) {
