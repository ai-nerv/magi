...
mod reduction {
    use super::super::*;
    use axon_proto::{StopReason, ToolResult};
    ...
    fn a_reattach_snapshot_does_not_replay_what_is_already_on_screen() {
        ...
        app.apply(HarnessEvent::SessionSnapshot {
            cursor: Cursor(2),
            session: axon_proto::SessionId::new("s"),
            entries: vec![
                Entry::User {
                    id: MessageId::new("m1"),
                    text: "asked".into(),
    ...
    fn an_error_event_becomes_a_visible_transcript_entry() {
        let app = app_with(vec![HarnessEvent::Error {
            cursor: Cursor(1),
            class: axon_proto::ErrorClass::Overload,
            message: "busy".into(),
        }]);
        match &app.entries()[0] {
            Entry::Assistant {
...
mod usage_tests {
    use super::super::*;
    use axon_proto::{ModelInfo, SessionId, StopReason, Usage};
...
mod onboarding_tests {
    use super::super::*;
    use axon_proto::{ModelChoice, ModelInfo, SessionId};
...
mod model_switch {
    ...
    fn info(name: &str) -> axon_proto::ModelInfo {
        axon_proto::ModelInfo {
            name: name.to_owned(),
...
mod stale_daemon {
    ...
    fn choice(name: &str, requirement: &str, wants: &[&str]) -> axon_proto::ModelChoice {
        axon_proto::ModelChoice {
            name: name.to_owned(),
            context_window: 100_000,
    ...
    fn a_key_this_process_can_see_is_not_a_key_to_go_and_set() {
        ...
        assert!(
            notices(&app).iter().any(|n| n.contains("axon stop")),
    ...
    fn a_key_nobody_has_set_still_says_to_set_it() {
        ...
        app.choices = vec![choice(
            "anthropic/m",
            "set AXON_NO_SUCH_KEY_ANYWHERE",
            &["AXON_NO_SUCH_KEY_ANYWHERE"],
        )];
        app.open_model_picker();
        let row = app
            .picker
            ...
            .current()
            .expect("a row");
        assert_eq!(row.detail, "set AXON_NO_SUCH_KEY_ANYWHERE");
