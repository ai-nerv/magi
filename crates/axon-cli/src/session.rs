//! What one axon calls itself, before anything else has an opinion.
//!
...
//! unique among the axons running now, and it must not depend on another program being
//! installed. So it is the process id and the clock, and it is never shown to anybody.
//!
//! **A name** is what a person and a sibling call this session: `axon/main/psi-omicron`. That is
//! [`atom`](crate::atom)'s to give, because atom holds the directory those names live in and can
...
//! to open a file. A session with no atom still has a journal.
...
///
/// Under axon's runtime directory, not atom's: this is the UI talking to its own session, and it
...
pub fn socket_for(project: &str, key: &str) -> std::path::PathBuf {
    ...
    socket_in(&runtime.join("axon").join(safe(project)), key)
}
...
mod tests {
    ...
    fn two_keys_from_one_process_are_still_two_keys() {
        ...
        // because two `axon -p` in a script share nothing else.
        assert_ne!(key(), key());
    }
    ...
    fn a_project_is_the_folder_rather_than_the_path() {
        ...
        // Otherwise `axon.project = ""` names a session after nothing.
    ...
    fn two_sessions_in_one_project_do_not_share_a_socket() {
        // The bug that started all of this: named after the *directory*, a second axon found
        // the first one already answering and joined it.
        assert_ne!(socket_for("axon", &key()), socket_for("axon", &key()));
    }
}
