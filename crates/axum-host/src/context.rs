...
/// [`axum_provider::retry`] classifies `Invalid` — neither retryable nor `Overflow` — so nothing
...
mod branch_tests {
    use super::*;
    use axum_proto::{MessageId, SessionId, Signatures};

    fn session(name: &str) -> (Session, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("axum-branch-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let session =
            Session::open(&dir.join("s.jsonl"), SessionId::new("s"), "/tmp", 0).expect("session");
        (session, dir)
...
mod repair_tests {
    ...
    fn session(name: &str) -> (Session, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("axum-repair-{}-{name}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let session =
            Session::open(&dir.join("s.jsonl"), SessionId::new("s"), "/tmp", 0).expect("session");
        (session, dir)
    ...
    fn calling(session: &mut Session, at: usize) -> Result<axum_proto::Cursor, JournalError> {
        session.commit(Entry::Assistant {
    ...
    fn call(
        ...
        at: usize,
        answered: bool,
    ) -> Result<axum_proto::Cursor, JournalError> {
        session.commit(Entry::Tool {
