...
use axum_proto::permit::{Action, Decision};
use axum_proto::{Cursor, HarnessEvent, ToolCallId};
...
impl axum_tools::approve::Approver for Asker {
    fn ask(&self, tool: &str, action: &Action) -> Decision {
        ...
        (self.publish)(HarnessEvent::PermissionAsked {
            ...
            tool: tool.to_owned(),
            action: action.clone(),
            offers: axum_tools::permit::Ledger::offers(action),
        });
...
mod tests {
    use super::*;
    use axum_tools::approve::Approver;
        ...
        let answering = std::thread::spawn(move || {
            ...
            for _ in 0..200 {
                ...
                pending.answer(
                    &id,
                    Decision::Allow {
                        scope: axum_proto::permit::Scope::Once,
