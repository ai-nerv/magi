...
//!
//! Everything a person chose *while using* axum used to be forgotten the moment the daemon
//! stopped. `/model` switched to a model and the next run went back to whatever `axum.model`
...
//! hand-editing — `axum.model` is still where a chosen default is *written down*, and this only
...
pub struct Chosen {
    /// The model, as `axum models` names it.
...
mod tests {
    ...
    fn the_record_sits_beside_the_journals_rather_than_in_the_config() {
        // Losing it costs a preference, not a setting. `axum.model` is still where a chosen
        // default is written down.
        let path = path();
        assert!(path.ends_with("axum/chosen.json"), "{}", path.display());
        assert!(
