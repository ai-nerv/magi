...
pub fn draw(frame: &mut Frame<'_>, app: &mut App, footer_data: &FooterData) -> u16 {
    ...
    // Where the footer name landed, which is what a press on it — opening the agents view — is
    // measured against, and where the hover-invert is drawn. The footer names the columns; only the
