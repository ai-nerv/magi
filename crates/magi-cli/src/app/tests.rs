...
mod reduction {
    use super::super::*;
    use magi_proto::{StopReason, ToolResult};
    ...
    fn a_reattach_snapshot_does_not_replay_what_is_already_on_screen() {
        ...
        app.apply(HarnessEvent::SessionSnapshot {
            cursor: Cursor(2),
            session: magi_proto::SessionId::new("s"),
            entries: vec![
                Entry::User {
                    id: MessageId::new("m1"),
                    text: "asked".into(),
    ...
    fn an_error_event_becomes_a_visible_transcript_entry() {
        let app = app_with(vec![HarnessEvent::Error {
            cursor: Cursor(1),
            class: magi_proto::ErrorClass::Overload,
            message: "busy".into(),
        }]);
        match &app.entries()[0] {
            Entry::Assistant {
...
mod usage_tests {
    use super::super::*;
    use magi_proto::{ModelInfo, SessionId, StopReason, Usage};
...
mod onboarding_tests {
    use super::super::*;
    use magi_proto::{ModelChoice, ModelInfo, SessionId};
...
mod model_switch {
    ...
    fn info(name: &str) -> magi_proto::ModelInfo {
        magi_proto::ModelInfo {
            name: name.to_owned(),
