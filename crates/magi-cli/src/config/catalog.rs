...
//! until the next refresh. What is fetched goes to a cache under `$XDG_CACHE_HOME/magi/models/`,
...
fn key_for(provider: &Provider) -> Option<String> {
    let magi_provider::provider::Auth::ApiKey { vars } = &provider.auth else {
        return None;
    };
...
/// rather than dropping the model — a model you cannot see is worse than one whose price magi
/// does not know.
#[must_use]
...
fn one(entry: &serde_json::Value) -> Option<Model> {
    ...
    Some(Model {
        ...
        input: vec![magi_provider::model::Modality::Text],
...
/// A price magi cannot read is no price rather than a wrong one.
fn cost(entry: &serde_json::Value) -> magi_model::Cost {
    let Some(pricing) = entry.get("pricing") else {
        return magi_model::Cost::default();
    };
...
mod tests {
    ...
    fn a_bare_entry_is_kept_rather_than_dropped() {
        // A model you cannot see is worse than one whose price magi does not know.
    ...
    fn a_declared_catalog_is_never_asked_about() {
        ...
        let mut providers = vec![magi_provider::provider::Provider {
            id: "fixed".into(),
            name: "Fixed".into(),
