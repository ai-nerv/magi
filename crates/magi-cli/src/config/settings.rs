...
pub fn adopt_ui(loaded: &Loaded) {
    ...
    let mut glyphs = magi_tui::glyph::Glyphs::default();
    glyphs.overlay(&|name| {
        ui.get(name)
    ...
    magi_tui::glyph::adopt(glyphs);

    let mut metrics = magi_tui::metric::Metrics::default();
    metrics.overlay(&|name| ui.get(name).and_then(serde_json::Value::as_u64));
    magi_tui::metric::adopt(metrics);
}
...
mod ui_tests {
    ...
    fn palette_of(source: &str) -> magi_tui::colour::Palette {
    ...
    fn a_value_of_the_wrong_kind_is_left_alone() {
        ...
        let chosen = palette_of("magi.ui.accent = 300\nmagi.ui.dim = 'grey'");
        assert_eq!(chosen.accent, magi_tui::colour::STOCK.accent);
        assert_eq!(chosen.dim, magi_tui::colour::STOCK.dim);
    }

    #[test]
    fn a_name_nothing_recognises_is_ignored_rather_than_refused() {
        // A config written for a later magi still starts this one.
        let loaded = from_lua("magi.ui.from_the_future = 3");
    ...
    fn the_three_kinds_are_named_apart() {
        ...
        use magi_tui::{colour::Palette, glyph::Glyphs, metric::Metrics};
...
mod environ_tests {
    ...
    fn a_table_of_strings_comes_through() {
        let seen = from(r#"magi.env = { PAGER = "cat", RUST_LOG = "warn" }"#);
...
mod decrypt_tests {
    ...
    /// What `magi.ui` says, as the overlay would read it.
    ...
    fn it_is_off_until_somebody_asks() {
        ...
        assert_eq!(magi_tui::metric::BUILT_IN.decrypt_ms, 0);
    }

    #[test]
    fn a_config_can_switch_it_on() {
        let loaded = from_lua("magi.ui.decrypt_ms = 900");
        let ui = ui_of(&loaded);
        let mut metrics = magi_tui::metric::Metrics::default();
        metrics.overlay(&|name| ui.get(name).and_then(serde_json::Value::as_u64));
    ...
    fn and_choose_what_it_scrambles_with() {
        let loaded = from_lua(r#"magi.ui.decrypt_pool = "01""#);
        let ui = ui_of(&loaded);
        let mut glyphs = magi_tui::glyph::Glyphs::default();
        glyphs.overlay(&|name| {
            ui.get(name)
