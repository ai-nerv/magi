...
fn a_conversation_renders_the_way_pi_lays_it_out() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            // A step in from the fill on both sides — and the fill itself stands one column
            // wider than the text, so the box holds the words rather than reading as a
            // highlight drawn behind them. See `transcript::frame::BLEED`.
            "    run the tests",
            "",
            "────────────────────────────────────────────",
...
/// Run with `cargo test -p magi-cli -- --ignored --nocapture visual`.
#[test]
