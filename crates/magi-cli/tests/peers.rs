...
//! peers magi ships — a shell in `sh`, and Lua in its own VM — through the same registry, and
...
fn a_peer_that_cannot_start_says_why() {
    // Found against a real model. The shipped config named the peer `magi` and trusted PATH to
...
fn a_peer_can_be_confined_by_configuration_alone() {
    // Open question 5, settled: magi needs no sandboxing subsystem because a process tool
    ...
    registry.register(Box::new(magi_tools::process::ProcessTool::new(
        ...
        vec![
            ...
            dir.display().to_string(),
            env!("CARGO_BIN_EXE_magi").to_owned(),
            "ext".into(),
            "shell".into(),
        ],
    )));
    ...
    let outside = registry.call(
        "bash",
        &serde_json::json!({ "command": "touch /etc/magi-should-not-exist" }),
        &ops,
        &Uncancelled,
    );
    assert!(
    ...
    assert!(
        !std::path::Path::new("/etc/magi-should-not-exist").exists(),
...
fn a_round_of_calls_to_different_peers_overlaps() {
    ...
    let sent: Vec<_> = ["one", "two"]
        ...
        .collect();
    assert!(
        sent.iter().all(magi_tools::Prepared::in_flight),
        "both went out"
    );
...
mod goodbye {
    ...
    fn marked(mark: &str) -> Vec<u32> {
        let needle = format!("MAGI_PEER_MARK={mark}");
    ...
    fn the_shell_a_peer_started_goes_with_it() {
        ...
        // shell it had opened. A machine running magi for a day carried a thousand of them,
        ...
        registry.register(Box::new(
            ...
            .with_env(
                [("MAGI_PEER_MARK".to_owned(), mark.clone())]
                    .into_iter()
                    .collect(),
            ),
        ));
