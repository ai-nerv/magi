/* A tool peer written against the documentation, not against magi's code.
 *
 ...
 * shares nothing: no magi crate, no CBOR library, no Rust. Every byte below was produced from
 ...
 * Check:  it is run by `crates/magi-cli/tests/conformance.rs`, which is the point of it.
 *
...
static void declare(void) {
    ...
    text("echo");
    text("description");
    text("Repeat what it is given, from a peer that shares no code with magi.");
    text("parameters");
    map(3);
    text("type");
    text("object");
